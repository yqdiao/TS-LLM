...
# 设置输入和输出文件夹路径
input_folder = "C:\\Users\\A\\Desktop\\160105\\混合模型预测结果"
output_folder = "C:\\Users\\A\\Desktop\\160105\\混合模型预测分析结果"
...
for file_name in input_files:
    ...
    # 构建输出文件名，例如将 "AR1和Market混合模型预测.csv" 转为 "AR1和Market混合模型预测分析.csv"
    base_name, ext = os.path.splitext(file_name)
    output_file_name = f"{base_name}分析{ext}"
    ...
    try:
        ...
        print(f"无法读取文件 {input_file_path}，错误信息：{e}")
        continue
    ...
    for i in range(n_stocks):
        ...
        if predicted_col_idx >= data_filtered.shape[1]:
            print(f"警告：文件 {file_name} 中股票 {i + 1} 的列索引超出范围，跳过该股票。")
            continue
    ...
    if regression_results.empty:
        print(f"文件 {file_name} 中没有有效的回归结果，跳过保存。")
        continue
    ...
    try:
        ...
        print(f"文件 {input_file_path} 的回归分析结果已保存到 {output_file_path}\n")
    except Exception as e:
        print(f"无法保存文件 {output_file_path}，错误信息：{e}")
        continue
