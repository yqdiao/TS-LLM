...
#todo: only need one column for time
#todo: remove na before processing data
...
for i in range(n_stocks):
    ...
    returns_col = data.iloc[:, 2 * i + 1] #why iloc?
    ...
    # 标准化收益率
    scaler = StandardScaler() #so all variables have unit variance
    ...
    for k in range(n_stocks):
        returns_individual = data.iloc[:, 2 * k + 1].dropna().values
        #returns_individual_scaled = scaler.fit_transform(returns_individual.values.reshape(-1, 1)).flatten()
        if len(returns_individual) > window_size:
                returns_all.append(returns_individual)
    #returns_all_scaled  = scaler.fit_transform(returns_all)
    ...
    for j in range(start_index + window_size, len(returns_col_scaled)):
        ...
        # 准备当前窗口的训练数据 #todo: X should be a n_stocks*30 matrix
        #X_train = np.array([returns_col_scaled[j - window_size:j]])
        X_train = np.array([x[j - window_size:j] for x in returns_all]).T
        #y_train = returns_col_scaled[j - window_size + 1:j + 1]
        y_train = returns_col[j - window_size + 1:j + 1]
        
        # 使用LassoCV进行10折交叉验证，增加max_iter和tol
        lasso_cv = LassoCV(cv=10, max_iter=50000, tol=0.1)
        ...
        if j + 1 < len(returns_col_scaled):
            X_test = np.array([x[j - window_size + 1:j + 1] for x in returns_all]).T
